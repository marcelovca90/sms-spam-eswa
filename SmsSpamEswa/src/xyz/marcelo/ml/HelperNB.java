...
public class HelperNB {
	...
	public static String[] buildInput(LinkedList<SmsMessage> data, int Nf, LinkedList<String> trainTotalTopWords) {
		...
		for (int i = 0; i < data.size(); i++) {
			ans[i] = "";
			for (int j = 0; j < Nf; j++)
				if (data.get(i).getWords().contains(trainTotalTopWords.get(j)))
					ans[i] += (trainTotalTopWords.get(j) + " ");
		}
		return ans;
	}
	...
	public static void run(String config, int Ld, int Nf, LinkedList<SmsMessage> trainData,
			LinkedList<SmsMessage> testData, LinkedList<String> trainHamTopWords, LinkedList<String> trainSpamTopWords,
			LinkedList<String> trainTotalTopWords) {
		...
		BayesianClassifier bayes = new BayesianClassifier(0, trainHamTopWords.toArray(new String[trainHamTopWords
				.size()]), trainSpamTopWords.toArray(new String[trainSpamTopWords.size()]));
